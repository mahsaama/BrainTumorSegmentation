...
from train_unet import fit_unet

Nclasses = 4
classes = np.arange(Nclasses)

# images lists
t1_list = sorted(glob.glob('../BRATS_2020/Training/*/*t1.nii.gz'))
t2_list = sorted(glob.glob('../BRATS_2020/Training/*/*t2.nii.gz'))
t1ce_list = sorted(glob.glob('../BRATS_2020/Training/*/*t1ce.nii.gz'))
flair_list = sorted(glob.glob('../BRATS_2020/Training/*/*flair.nii.gz'))
seg_list = sorted(glob.glob('../BRATS_2020/Training/*/*seg.nii.gz'))
...
train_gen = DataGenerator(sets['train'], batch_size=batch_size, n_classes=n_classes, augmentation=True, patch_size=patch_size)
valid_gen = DataGenerator(sets['valid'], batch_size=batch_size, n_classes=n_classes, augmentation=True, patch_size=patch_size)
    
# train the vox2vox model
# h = fit(train_gen, valid_gen, alpha, n_epochs)

# train the unet model
h = fit_unet(train_gen, valid_gen, alpha, n_epochs)
